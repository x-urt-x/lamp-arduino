...
void Strip::apply_br() {
	...
	LOG_USB_BR("apply_br - _cur_cutoff_order_len %d\n", _cur_cutoff_order_len);
	LOG_USB_BR("apply_br - _cur_cutoff_imm_len %d\n", _cur_cutoff_imm_len);

	float br_add = 0;
	int to_process = _cur_cutoff_order_len + _cur_cutoff_imm_len; //счетчик необработанных светодиодов 
	float br_avg_per = 0;
	for (byte i = 0; i < _cur_cutoff_order_len; i++)
	{
		Color_str* led = _leds_arr + _cur_cutoff_order[i]; //текущий светодиод для обработки
	...
	for (byte i = 0; i < _cur_cutoff_imm_len; i++) //i - индекс текущего светодиода, порядок определен константой 
	{
		LOG_USB_BR("centr cyc i=%d\t", i);
		LOG_USB_BR("full\t");
		_leds_arr[_cur_cutoff_imm[i]].map((byte_round_up(br_max) + br_add));
		to_process--;
		br_add += (br_add - int(br_add)) / to_process; //переносим дробную часть на оставшиеся
		LOG_USB_BR("\n");
	}
